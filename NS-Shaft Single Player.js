var game = new Phaser.Game(600, 600, Phaser.AUTO, '',
    { preload: preload, create: create, update: update }); // 在網頁上的長 寬
...
function preload () {
    ...
    game.load.baseURL = 'https://wacamoto.github.io/NS-Shaft-Tutorial/assets/'; // 代換自己的線上.io網址
...
function createPlatforms () {
    if(game.time.now > lastTime + 600) { // 每隔600毫秒創造一個新平台
...
function createOnePlatform () {
    ...
    var platform;
    var x = Math.random()*(600 - 96 - 40) + 20; // (邊界長度 - 平台寬像素 - ?) + ?
    var y = 600; // 邊界寬度
...
function createPlayer() {
    player = game.add.sprite(300, 150, 'player'); // 玩家起始位置
...
function createTextsBoard () {
    ...
    text1 = game.add.text(10, 10, '', style); // 地下幾個平台字樣
    text2 = game.add.text(550, 10, '', style); // 生命數字樣
    text3 = game.add.text(210, 400, 'Enter 重新開始', style); // 重新開始字樣
    text3.visible = false;
}

function updatePlayer () {
    if(keyboard.left.isDown) { // isDown代表是否有按下左鍵
        player.body.velocity.x = -300; // 按左鍵 玩家左移速度
    } else if(keyboard.right.isDown) {
        player.body.velocity.x = 300; // 按右鍵 玩家右移速度
    } else {
...
function updatePlatforms () {
    for(var i=0; i<platforms.length; i++) {
        ...
        platform.body.position.y -= 2; // 平台往上移
        if(platform.body.position.y <= -20) { // 若平台上移到天花板了
            platform.destroy();
...
function trampolineEffect(player, platform) {
    ...
    player.body.velocity.y = -350; // 彈跳床使玩家上移速度
}
